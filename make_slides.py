...
# ── Palette ────────────────────────────────────────────────────────────────────
DARK_BG    = RGBColor(0xEB, 0xF1, 0xFF)   # light lavender-blue  — slide background
MID_BG     = RGBColor(0xFF, 0xFF, 0xFF)   # white                — card / panel backgrounds
ACCENT     = RGBColor(0x1D, 0x4E, 0xD8)   # deep blue            — bars, table headers
HIGHLIGHT  = RGBColor(0x15, 0x57, 0xB0)   # medium blue          — headings on light bg
GREEN      = RGBColor(0x05, 0x7A, 0x55)   # darker emerald       — success / selected
ORANGE     = RGBColor(0xB4, 0x5A, 0x09)   # darker amber         — warnings / notes
RED        = RGBColor(0xC0, 0x20, 0x20)   # darker red           — drops / errors
WHITE      = RGBColor(0xFF, 0xFF, 0xFF)   # pure white           — text ON dark elements
LIGHT_GREY = RGBColor(0x1E, 0x29, 0x3B)   # near-black navy      — main body text
SUBTEXT    = RGBColor(0x52, 0x62, 0x7A)   # medium blue-grey     — captions / secondary
...
def slide_title(prs):
    ...
    add_rect(s, 0, 0, Inches(5.2), SLIDE_H, ACCENT)   # keep left panel dark for contrast
    add_rect(s, 0, 0, Inches(0.18), SLIDE_H, RGBColor(0x93, 0xC5, 0xFD))  # light blue stripe
    ...
    add_textbox(s, "AI Lab Project — Spring 2026\nFAST NUCES BS CS",
                ...
                font_size=15, color=RGBColor(0xBF, 0xD7, 0xFF), align=PP_ALIGN.LEFT)
...
def slide_dataset_overview(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        bg = ACCENT if ri == 0 else (MID_BG if ri % 2 == 1 else RGBColor(0xE0, 0xEA, 0xFF))
...
def slide_dataset_expansion(prs):
    ...
    boxes = [
        ...
        ("4 Rows\n1 positive\n3 negative", Inches(4.7), Inches(1.9), Inches(2.0), Inches(1.3), RGBColor(0xD1, 0xFA, 0xE5)),
    ]
...
def slide_eda_distributions(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        bg = ACCENT if ri == 0 else (MID_BG if ri % 2 else RGBColor(0xE0, 0xEA, 0xFF))
...
def slide_eda_template_wins(prs):
    ...
    templates = [
        ...
        ("inference",            7445, RGBColor(0x7C, 0x3A, 0xED)),
        ("what_information",     6460, RED),
        ("main_idea_about",      5344, RGBColor(0xD9, 0x77, 0x06)),
        ("main_idea_title",      4553, RGBColor(0x05, 0x96, 0x69)),
        ("not_true",             4315, RGBColor(0xBE, 0x18, 0x5D)),
...
def slide_eda_tfidf_vs_onehot(prs):
    ...
    reasons = [
        ...
        (RGBColor(0x7C, 0x3A, 0xED), "Better fit for LinearSVC and Logistic Regression",
...
def slide_statistical_analysis(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        bg = ACCENT if ri == 0 else (RGBColor(0xE0, 0xEA, 0xFF) if ri % 2 == 0 else MID_BG)
        if ri == 5: bg = RGBColor(0xD1, 0xFA, 0xE5)
    ...
    for ri, row in enumerate(rows2):
        ...
        bg = ACCENT if ri == 0 else (MID_BG if ri == 1 else RGBColor(0xFE, 0xE2, 0xE2))
...
def slide_model_a_selection(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        is_best = ri == 7
        bg = ACCENT if ri == 0 else (RGBColor(0xD1, 0xFA, 0xE5) if is_best else (MID_BG if ri % 2 else RGBColor(0xE0, 0xEA, 0xFF)))
...
def slide_model_a_qgen_features(prs):
    ...
    groups = [
        ...
        ("Overlap Features", RGBColor(0x7C, 0x3A, 0xED), [
        ...
        ("Answer Type One-hot (4)", RGBColor(0xD9, 0x77, 0x06), [
    ...
    for ci, col_groups in enumerate(col_data):
        ...
        for group_name, col, items in col_groups:
            ...
            add_textbox(s, group_name, x + Inches(0.1), Inches(y_off + 0.04),
                        Inches(6.0), Inches(0.3), font_size=13, bold=True, color=WHITE)
            y_off += 0.38
            for item in items:
...
def slide_model_b_overview(prs):
    ...
    for i, (num, label, col) in enumerate(cols_info):
        x = Inches(0.5 + i * 3.2)
        add_rect(s, x, Inches(6.6), Inches(2.9), Inches(0.82), RGBColor(0xE0, 0xEA, 0xFF))
...
def slide_model_b_training(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        is_rf = ri == 2
        bg = ACCENT if ri == 0 else (RGBColor(0xD1, 0xFA, 0xE5) if is_rf else MID_BG)
    ...
    # Winner callout
    add_rect(s, Inches(0.35), Inches(4.4), Inches(12.9), Inches(1.05), RGBColor(0xD1, 0xFA, 0xE5))
...
def slide_model_b_hint_results(prs):
    ...
    add_rect(s, Inches(0.4), Inches(5.7), Inches(6.1), Inches(1.55), RGBColor(0xD1, 0xFA, 0xE5))
...
def slide_model_b_honest(prs):
    ...
    # Insight box
    add_rect(s, Inches(0.5), Inches(5.0), Inches(12.3), Inches(2.1), RGBColor(0xFE, 0xE2, 0xE2))
...
def slide_results_summary(prs):
    ...
    for ri, row in enumerate(rows):
        ...
        special = ri in [4, 6]
        bg = ACCENT if ri == 0 else (RGBColor(0xD1, 0xFA, 0xE5) if special else (MID_BG if ri % 2 else RGBColor(0xE0, 0xEA, 0xFF)))
...
def slide_metric_interpretation(prs):
    ...
    points = [
        ...
        (RGBColor(0x7C, 0x3A, 0xED), "Silhouette",
...
def slide_system_architecture(prs):
    ...
    for i, step in enumerate(path2):
        x = Inches(0.35 + i * 2.55)
        add_rect(s, x, Inches(3.72), Inches(2.35), Inches(1.1), RGBColor(0xD1, 0xFA, 0xE5))
...
def slide_closing(prs):
    ...
    takeaways = [
        ...
        (RGBColor(0x7C, 0x3A, 0xED), "Traditional ML has limits on this task",
